    ...
    init {
        "calling execute with a valid user account" should {
            transaction {
                ...
                val getUserAccountResult = GetUserAccountService.execute(mapOf(Pair("userId", userAccount.idValue.toString())))
        ...
        "calling execute with a valid user account email" should {
            transaction {
                val userAccount = UserAccount.all().first()
                val getUserAccountResult = GetUserAccountService.execute(mapOf(Pair("email", userAccount.userMetadata.email)))
                getUserAccountResult.result shouldBe SOAResultType.SUCCESS
                getUserAccountResult.data?.idValue shouldBe userAccount.idValue
            }
        }
        "calling execute with a valid user account apiKey" should {
            transaction {
                val userAccount = UserAccount.all().first()
                val getUserAccountResult = GetUserAccountService.execute(mapOf(Pair("apiKey", userAccount.apiCreds.apiKey)))
                getUserAccountResult.result shouldBe SOAResultType.SUCCESS
                getUserAccountResult.data?.idValue shouldBe userAccount.idValue
            }
        }
